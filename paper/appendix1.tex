...
\right).
\end{equation}
The message function $\psi^{(\ell)}$ says how much node $j$ should influence node $i$, the update function $\phi^{(\ell)}$ combines that information with the current state of node $i$, and the edge features $\mathbf{e}_{ij}$ allow the connection strength or type to modulate that message. After several layers, the node states are pooled to form a graph-level vector that can be used for prediction.
