...
from .record_views import SUBGROUP_ORDER, materialize_graph_view_records
...
def run_saved_graph_sweep(
    *,
    ...
    view_records_paths: dict[str, Path] = {"full": records_path}
    view_reports: dict[str, dict] = {}
    if "interface" in graph_views:
        view_dir = out_dir / "record_views"
        ...
        view_records_paths["interface"] = interface_path
        view_reports["interface"] = interface_report

    rows: list[dict] = []
    run_t0 = time.perf_counter()
    total_configs = len(graph_views) * len(modes) * len(latent_dims) * len(supervision_modes)
    config_idx = 0

    for graph_view in graph_views:
        view_records = view_records_paths[graph_view]
        for mode in modes:
