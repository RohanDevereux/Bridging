...
def _annotate_source_row_index(rows: list[dict[str, str]]) -> tuple[list[dict[str, str]], bool]:
    annotated = []
    added = False
    for idx, row in enumerate(rows):
        row_copy = dict(row)
        if "source_row_index" not in row_copy:
            row_copy["source_row_index"] = str(idx)
            added = True
...
def preshard_dataset(
    dataset: str | Path,
    shard_dir: str | Path | None = None,
    num_shards: int = 2,
    dedup_by_pdb: bool = True,
) -> Path:
    ...
    if dedup_by_pdb:
        ...
        dropped_dupes = 0
    rows, added_source_row_index = _annotate_source_row_index(rows)
    if added_source_row_index:
        fieldnames = list(fieldnames) + ["source_row_index"]
...
def main() -> None:
    ...
    parser.add_argument(
        "--no-dedup-by-pdb",
        action="store_true",
        help="Keep repeated PDB rows instead of dropping duplicate PDB identifiers before sharding.",
    )
    ...
    preshard_dataset(
        ...
        dedup_by_pdb=not args.no_dedup_by_pdb,
    )
